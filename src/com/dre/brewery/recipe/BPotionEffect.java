...
import com.dre.brewery.api.events.PlayerEffectEvent.EffectType;
...
public class BPotionEffect implements BEffect {
	...
	public BPotionEffect(PotionEffectType type, short minlvl, short maxlvl, short minduration, short maxduration, boolean hidden) {
		this.type = type;
		this.minlvl = minlvl;
		this.maxlvl = maxlvl;
	...
	public BPotionEffect(String effectString) {
	...
	public void apply(int quality, Player player) {
		PotionEffect peffect = generateEffect(quality);
		PlayerEffectEvent event = new PlayerEffectEvent(player, EffectType.DRINK, Arrays.asList(new PotionEffect[] {peffect}));
		P.p.getServer().getPluginManager().callEvent(event);
		List<PotionEffect> effects = event.getEffects();
